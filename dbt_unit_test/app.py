...
def run(tests, batches, log_level):
    ...
    model = [f'+{test}_model+' for test in tests.split(',')] if tests else [f'+path:{models_dir}/{unit_test_dir}+']
    seed = [f'+{test}_model+' for test in tests.split(',')] if tests else [f'+path:{seed_dir}/{unit_test_dir}+']
